...
async function createCompanyRoom({ name, password, adminPassword }, onProgress) {
  ...
  if (typeof logCompanyAuditEvent === 'function') {
    logCompanyAuditEvent('company', name || 'Company', 'created', []).catch((err) => console.error('audit log:', err));
  }
...
async function leaveCompanyRoom() {
  ...
  // to, so this has to happen while this device still remembers being in
  // one. Awaited, not fire-and-forget, so it can't lose the race against
  // the deleteSetting calls right after it (both are async).
  if (room && typeof logCompanyAuditEvent === 'function') {
    await logCompanyAuditEvent('company', room.name || 'Company', 'left', []).catch((err) => console.error('audit log:', err));
  }
...
// Legacy company-wide "which projects count toward the dashboard" list --
// replaced by a per-person one (storage.js's getManagerDashboardExcluded
// ProjectIds/saveManagerDashboardExcludedProjectIds, synced per-user via
// pushUserLayout/pullUserLayout below, same as favorites/folder layout).
// Nothing writes to this anymore; it's only read once, as the starting
// point for a person who hasn't customized their own list yet, so whatever
// an admin had already curated here isn't just thrown away for everyone
...
async function pullUserLayout(code, userName) {
  ...
  // Older synced copies (pushed before this field existed) won't have it --
